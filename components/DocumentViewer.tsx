    ...
    <section className="card overflow-hidden">
      {files.length > 1 ? (
        <div
          ...
          className="flex overflow-x-auto border-b border-rule [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
          {files.map((f, i) => (
            <button
              key={f.id}
      ...
      <header className="flex items-baseline gap-3 border-b border-rule px-4 py-3 sm:px-5">
      ...
      <div className={clsx("w-full overflow-hidden border-b border-rule bg-wash", FRAME)}>
        {preview.mode === "pdf" ? (
          <object
            data={url}
          ...
          >
            ...
            <div className="flex h-full w-full flex-col">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-rule bg-paper px-4 py-3">
        ...
        {preview.mode === "text" ? (
          <div className="h-full w-full overflow-y-auto overflow-x-hidden bg-paper px-5 py-6 sm:px-8 sm:py-8">
            <pre className="mx-auto max-w-measure whitespace-pre-wrap break-words font-mono text-[12.5px] leading-[1.7]">
              {preview.text}
            </pre>
          </div>
        ) : null}
        ...
        {preview.mode === "unsupported" ? (
          <div className="flex h-full w-full flex-col items-center justify-center gap-4 bg-wash px-6 py-10 text-center">
            <span className="border border-rule bg-paper px-3 py-1.5 font-mono text-[11px] uppercase tracking-[0.18em] text-muted">
              {format.ext || "fichier"}
            </span>
      ...
      {preview.mode !== "unsupported" && preview.warning ? (
        <p className="border-b border-rule2 bg-wash px-4 py-2.5 text-[12px] leading-snug text-muted sm:px-5">
          {preview.warning}
        </p>
      ) : null}
